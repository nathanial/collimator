...
```
-/
@[inline] def left' (α β : Type u) : Prism' (Sum α β) α := left

/-- Prism focusing the right branch of a sum (polymorphic version). -/
@[inline] def right {α β γ : Type u} :
...
```
-/
@[inline] def right' (α β : Type u) : Prism' (Sum α β) β := right
