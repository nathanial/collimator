...
private def case_nestedTuples : TestCase := {
  ...
  run := do
    ...
    -- Set a new value
    let updated := set (optic := Lens) composed "world" data
...
private def case_companyZipCode : TestCase := {
  ...
  run := do
    ...
    -- Build lenses for each field
    let departmentsLens : Lens' Company (List Department) :=
      lens' (fun c => c.departments) (fun c depts => { c with departments := depts })

    let employeesLens : Lens' Department (List Employee) :=
      lens' (fun d => d.employees) (fun d emps => { d with employees := emps })

    let addressLens : Lens' Employee Address :=
      lens' (fun e => e.address) (fun e addr => { e with address := addr })

    let zipCodeLens : Lens' Address String :=
      lens' (fun a => a.zipCode) (fun a zip => { a with zipCode := zip })
    ...
    let newZip := view (optic := fun s a => Lens s s a a) companyToZip modified
    ...
    let updated := set (optic := Lens) companyToZip "54321" company
    let finalZip := view (optic := fun s a => Lens s s a a) companyToZip updated
...
private def case_isoTraversalComposition : TestCase := {
  ...
  run := do
    ...
    let empty := ""
    let emptyChars := toListTraversal stringCharsTraversal empty
...
private def case_traversalPrismComposition : TestCase := {
  ...
  run := do
    ...
    let remote := employees & finalTraversal %~ (fun _ => "Remote")
    let remoteCities := toListTraversal finalTraversal remote
...
private def case_traversalPrismSum : TestCase := {
  ...
  run := do
    ...
    let raised := results & finalTraversal %~ (fun s => s + s / 10)
    let newSalaries := toListTraversal finalTraversal raised
...
private def case_affineTraversal : TestCase := {
  ...
  run := do
    ...
    let modified := employees & finalAffine %~ (fun _ => "SF")
    ...
    let stillEmpty := empty & finalAffine .~ "Denver"
...
private def case_affineViaAt : TestCase := {
  ...
  run := do
    ...
    let raised := employees & finalAffine %~ (· + 10000)
    ...
    let unchanged := employees & outOfBounds %~ (fun e => { e with salary := 0 })
...
private def case_foldAggregations : TestCase := {
  ...
  run := do
    ...
    -- Build traversals for deep collection
    let departmentsLens : Lens' Company (List Department) :=
      lens' (fun c => c.departments) (fun c depts => { c with departments := depts })
    ...
    -- Build a Fold by composing Lenses from inside out
    ...
    let highEarners := allEmployees.filter (fun e => e.salary > 95000)
    ensureEq "High earners count" 2 highEarners.length
    ensureEq "High earners names" ["Alice", "Bob"] (highEarners.map (·.name))
...
private def case_ultimateComposition : TestCase := {
  ...
  run := do
    ...
    let nameLens : Lens' Employee String :=
      lens' (fun e => e.name) (fun e n => { e with name := n })
    ...
    let allCaps := company & companyToAllNameChars %~ Char.toUpper
    ...
    let replacedAs := company & aCharsOnly %~ (fun _ => 'A')
...
private def case_nestedOptions : TestCase := {
  ...
  run := do
    ...
    let ceoLens : Lens' CompanyWithCEO (Option Employee) :=
      lens' (fun c => c.ceo) (fun c ceo => { c with ceo := ceo })
    ...
    let movedCEO := companyWithCEO & ceoToCity .~ "Boston"
...
private def case_allSalaries : TestCase := {
  ...
  run := do
    ...
    let raised := company & companyToAllSalaries %~ (fun sal => sal + (sal / 10))
    ...
    let normalized := company & companyToAllSalaries %~ (fun _ => 100000)
