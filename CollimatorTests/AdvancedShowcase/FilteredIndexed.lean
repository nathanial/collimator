...
private def case_filteredBasics : TestCase := {
  ...
  run := do
    ...
    let result1 := Traversal.over' (filteredList (fun x => x % 2 == 0)) (· * 10) input1
    ensureEq "even filter" [1, 20, 3, 40, 5, 60] result1

    -- Filter by range
    let input2 : List Nat := [5, 15, 25, 45, 55, 65]
    let result2 := Traversal.over' (filteredList (fun x => x > 10 && x < 50)) (· + 1000) input2
    ensureEq "range filter" [5, 1015, 1025, 1045, 55, 65] result2

    -- Filter odds
    let result3 := Traversal.over' (filteredList (fun x => x % 2 == 1)) (· + 100) input1
...
private def case_filteredEdgeCases : TestCase := {
  ...
  run := do
    ...
    -- Empty list
    ensureEq "empty list" ([] : List Nat) (Traversal.over' evenFilter (· * 100) [])

    -- No matches
    let odds : List Nat := [1, 3, 5, 7]
    ensureEq "no matches" odds (Traversal.over' evenFilter (· * 100) odds)

    -- All match
    let evens : List Nat := [2, 4, 6]
    ensureEq "all match" [200, 400, 600] (Traversal.over' evenFilter (· * 100) evens)

    -- Single element
    ensureEq "single match" [52] (Traversal.over' evenFilter (· + 10) [42])
    ensureEq "single no match" [43] (Traversal.over' evenFilter (· + 10) [43])
...
private def case_filteredWithLens : TestCase := {
  ...
  run := do
    ...
    let restocked : List Product := Traversal.over' expensiveQty (· + 50) inventory
    ...
    let discounted : List Product := Traversal.over' lowStockPrice (fun p => p * 80 / 100) inventory
...
private def case_indexedBasics : TestCase := {
  ...
  run := do
    ...
    let result1 := Traversal.over' (ifilteredList (fun i _ => i % 2 == 0)) (· * 10) [1, 2, 3, 4, 5, 6]
    ensureEq "even indices" [10, 2, 30, 4, 50, 6] result1

    -- Modify odd indices only
    let result2 := Traversal.over' (ifilteredList (fun i _ => i % 2 == 1)) (· + 100) [1, 2, 3, 4, 5, 6]
    ensureEq "odd indices" [1, 102, 3, 104, 5, 106] result2

    -- First 3 elements
    let result3 := Traversal.over' (ifilteredList (fun i _ => i < 3)) (· * 10) [1, 2, 3, 4, 5, 6]
...
private def case_indexedIx : TestCase := {
  ...
  run := do
    ...
    let result1 := Traversal.over' (ix 3) (· * 10) input
    ensureEq "ix modify" [10, 20, 30, 400, 50] result1

    -- Out of bounds (no-op)
    let result2 := Traversal.over' (ix 10) (· * 999) input
    ensureEq "ix out of bounds" input result2

    -- ix on empty
    let result3 := Traversal.over' (ix 0) (· + 100) ([] : List Nat)
    ensureEq "ix on empty" ([] : List Nat) result3

    -- Multiple ix operations
    let result4 := input
      |> Traversal.over' (ix 0) (· + 10)
      |> Traversal.over' (ix 2) (· + 20)
      |> Traversal.over' (ix 4) (· + 30)
...
private def case_indexedAtLens : TestCase := {
  ...
  run := do
    ...
    ensureEq "view at 2" (some 30) (Collimator.view' at2 input)
    ensureEq "view out of bounds" (none : Option Nat) (Collimator.view' at10 input)
    ensureEq "view empty" (none : Option Nat) (Collimator.view' at0 ([] : List Nat))

    -- Set at valid index
    ensureEq "set at 2" [10, 20, 300, 40, 50] (Collimator.set' at2 (some 300) input)

    -- Set out of bounds (no-op)
    ensureEq "set out of bounds" input (Collimator.set' at10 (some 999) input)

    -- Over with Option.map
    let result : List Nat := Collimator.over' at1 (Option.map (· * 10)) input
...
private def case_filteredIndexedComplex : TestCase := {
  ...
  run := do
    ...
    let result1 := Traversal.over' (ifilteredList (fun i v => i % 2 == 1 && v % 2 == 0)) (· * 100) input
    ...
    let result2 := Traversal.over' (ifilteredList (fun i v => v > i)) (· * 2) input2
    ensureEq "value > index" [0, 10, 1, 16, 2, 20, 3] result2

    -- Value == index^2 (perfect squares)
    let squares : List Nat := [0, 1, 4, 9, 16, 25, 36]
    let result3 := Traversal.over' (ifilteredList (fun i v => v == i * i)) (· + 1000) squares
...
private def case_realworldArrayUpdates : TestCase := {
  ...
  run := do
    ...
    let result1 := Traversal.over' (ifilteredList (fun i _ => i % 2 == 0)) (· + 10) [1, 2, 3, 4, 5, 6, 7, 8]
    ensureEq "even positions" [11, 2, 13, 4, 15, 6, 17, 8] result1

    -- Zero out negatives
    let input2 : List Int := [-5, 10, -3, 0, 7, -2, 15]
    let result2 := Traversal.over' (filteredList (fun x : Int => x < 0)) (fun _ => 0) input2
    ensureEq "zero negatives" [0, 10, 0, 0, 7, 0, 15] result2

    -- Clamp large values
    let input3 : List Nat := [10, 150, 30, 200, 50, 180]
    let result3 := Traversal.over' (filteredList (fun x : Nat => x > 100)) (fun _ => 100) input3
...
private def case_realworldConditionalBatch : TestCase := {
  ...
  run := do
    ...
    let discounted : List Product := Traversal.over' expensivePrice (fun p => p * 80 / 100) inventory
    ensure (discounted[1]!.price == 120) "discount gadget"
    ensure (discounted[2]!.price == 160) "discount premium"
    ensure (discounted[0]!.price == 50) "widget unchanged"
    ...
    let lowStock : Traversal' (List Product) Product := filteredList (fun p => p.quantity < 10)
    let lowStockPrice : Traversal' (List Product) Nat := lowStock ∘ priceLens
    let taxed : List Product := Traversal.over' lowStockPrice (fun p => p * 110 / 100) inventory
...
private def case_realworldSparseArrays : TestCase := {
  ...
  run := do
    ...
    ensureEq "double" [0, 10, 0, 0, 6, 0, 14, 0] (Traversal.over' nonZero (· * 2) input)

    -- Normalize to 1
    ensureEq "normalize" [0, 1, 0, 0, 1, 0, 1, 0] (Traversal.over' nonZero (fun _ => 1) input)

    -- Non-zero at even indices
    let input2 : List Nat := [0, 5, 10, 0, 20, 0, 30, 0]
    let result := Traversal.over' (ifilteredList (fun i v => v != 0 && i % 2 == 0)) (· + 1000) input2
...
private def case_performance : TestCase := {
  ...
  run := do
    ...
    let result := Traversal.over' evenLarge (· * 10) input
...
private def case_composition : TestCase := {
  ...
  run := do
    ...
    let evens : Traversal' (List Nat) Nat := filteredList (fun x => x % 2 == 0)
    ensureEq "evens * 2" [1, 4, 3, 8, 5, 12] (Traversal.over' evens (· * 2) input)
    ensureEq "evens + 100" [1, 102, 3, 104, 5, 106] (Traversal.over' evens (· + 100) input)

    -- Negated filter (NOT even = odd)
    let notEven : Traversal' (List Nat) Nat := filteredList (fun x => x % 2 != 0)
    ensureEq "negated filter" [11, 2, 13, 4, 15, 6] (Traversal.over' notEven (· + 10) input)

    -- Union: even OR > 4
    let union : Traversal' (List Nat) Nat := filteredList (fun x => x % 2 == 0 || x > 4)
    ensureEq "union" [1, 20, 3, 40, 50, 60] (Traversal.over' union (· * 10) input)

    -- Intersection: even AND > 3
    let intersection : Traversal' (List Nat) Nat := filteredList (fun x => x % 2 == 0 && x > 3)
    ensureEq "intersection" [1, 2, 3, 104, 5, 106] (Traversal.over' intersection (· + 100) input)
