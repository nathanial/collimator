...
def filteredListTests : List TestCase := [
  ...
  { name := "filteredList: empty list"
    run := do
      let result := over (filteredList (· > 0)) (· * 2) ([] : List Int)
...
def ifilteredListTests : List TestCase := [
  { name := "ifilteredList: modify even indices"
    run := do
      let result := over (ifilteredList (fun i _ => i % 2 == 0)) String.toUpper ["a", "b", "c", "d"]
...
def headTests : List TestCase := [
  { name := "_head: preview non-empty"
    run := do
      let result := preview _head [1, 2, 3]
      ensureEq "head is 1" (some 1) result
  },
  { name := "_head: preview empty"
    run := do
      let result := preview _head ([] : List Int)
      ensureEq "head is none" none result
  },
  { name := "_head: over non-empty"
    run := do
      let result := over _head (· * 10) [1, 2, 3]
      ensureEq "head modified" [10, 2, 3] result
  },
  { name := "_head: over empty"
    run := do
      let result := over _head (· * 10) ([] : List Int)
      ensureEq "empty unchanged" [] result
  },
  { name := "_head: set value"
    run := do
      let result := set _head 100 [1, 2, 3]
...
def lastTests : List TestCase := [
  { name := "_last: preview non-empty"
    run := do
      let result := preview _last [1, 2, 3]
      ensureEq "last is 3" (some 3) result
  },
  { name := "_last: preview singleton"
    run := do
      let result := preview _last [42]
      ensureEq "single element" (some 42) result
  },
  { name := "_last: preview empty"
    run := do
      let result := preview _last ([] : List Int)
      ensureEq "last is none" none result
  },
  { name := "_last: over non-empty"
    run := do
      let result := over _last (· * 10) [1, 2, 3]
...
def prismUtilityTests : List TestCase := [
  ...
  { name := "failing: over is identity"
    run := do
      let result := over (failing : Prism' Int Int) (· * 2) 42
      ensureEq "unchanged" 42 result
  },
  { name := "prismFromPartial: even numbers"
    run := do
      let evenPrism := prismFromPartial
...
def orElseTests : List TestCase := [
  { name := "orElse: first matches"
    run := do
      let p1 := prismFromPartial (fun n : Int => if n > 0 then some n else none) id
      let p2 := prismFromPartial (fun n : Int => if n < 0 then some n else none) id
      let combined := orElse p1 p2
      ensureEq "positive wins" (some 5) (preview combined 5)
  },
  { name := "orElse: second matches"
    run := do
      ...
      let p2 := prismFromPartial (fun n : Int => if n < 0 then some n else none) id
      let combined := orElse p1 p2
      ensureEq "negative wins" (some (-3)) (preview combined (-3))
  },
  { name := "orElse: neither matches"
    run := do
      let p1 := prismFromPartial (fun n : Int => if n > 10 then some n else none) id
      let p2 := prismFromPartial (fun n : Int => if n < -10 then some n else none) id
      let combined := orElse p1 p2
      ensureEq "zero fails both" none (preview combined 0)
  }
]
...
def helperTests : List TestCase := [
  { name := "first': explicit tuple lens"
    run := do
      let pair := (10, "hello")
      let result := view (Helpers.first' Int String) pair
      ensureEq "view first" 10 result
  },
  { name := "second': explicit tuple lens"
    run := do
      let pair := (10, "hello")
      let result := view (Helpers.second' Int String) pair
      ensureEq "view second" "hello" result
  },
  { name := "some': explicit option prism"
    run := do
      let opt : Option Int := some 42
      let result := preview (Helpers.some' Int) opt
      ensureEq "preview some" (some 42) result
  },
  ...
  { name := "lensOf: build lens with explicit types"
    run := do
      let xLens := Helpers.lensOf TestPoint Int (·.x) (fun p x => { p with x := x })
  ...
  { name := "prismOf: build prism with explicit types"
    run := do
      let positivePrism := Helpers.prismOf Int Int id (fun n => if n > 0 then some n else none)
      ensureEq "positive matches" (some 5) (preview' positivePrism 5)
      ensureEq "negative fails" none (preview' positivePrism (-3))
  }
]
